...
[ -f "$UFS_WRITEBOOSTER_CATCHUP_PATCH" ] || error "UFS/WriteBooster catch-up: not backported for kernel ${KERNEL_VERSION} yet (expected ${UFS_WRITEBOOSTER_CATCHUP_PATCH}) — this feature should have been gated out before reaching here (check run_tuning()'s support map)."
